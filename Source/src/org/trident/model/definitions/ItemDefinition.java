...
public class ItemDefinition {
	 ...
	 */
	private static final String FILE_DIRECTORY = "./data/definitions/item_definitions.txt";
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				case "examine":
					...
					break;
				case "archery":
					definition.archery = Integer.valueOf(value);
					break;
				case "weight":
	...
	public static void loadPrices() {
		try {
			Scanner s = new Scanner(new File("data/config/prices.txt"));
			while (s.hasNextLine()) {
				String[] line = s.nextLine().split(" ");
				ItemDefinition def = definitions[Integer.parseInt(line[0])];
				if (def != null)
					def.value = Integer.parseInt(line[1]);
			}
			s.close();
		} catch (IOException e) {
			e.printStackTrace();
