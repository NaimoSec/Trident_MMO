package org.trident.model.container;
...
public abstract class ItemContainer {
	...
	public ItemContainer switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		...
		if(this instanceof Shop) {
			boolean genStore = ((Shop)this).getId() == Shop.GENERAL_STORE;
			delete(item, slot, refresh, to);
			if(!genStore && getAmount(item.getId()) <= 0)
				add(new Item(item.getId(), 0));
		}
		to.add(item);
	...
	public ItemContainer add(Item item, boolean refresh) {
		if (item == null)
			return this;
		if(item.getId() == 995 && getAmount(995) + item.getAmount() >= Integer.MAX_VALUE)
			return this;
		if(this instanceof Bank)
			Achievements.handleAchievement(getPlayer(), Achievements.Tasks.TASK9);
		...
		if (ItemDefinition.forId(item.getId()).isStackable() || stackType() == StackType.STACKS) {
			...
			if (slot == -1)
				slot = getEmptySlot();
			if (slot == -1) {
				GroundItemManager.spawnGroundItem(player, new GroundItem(item, player.getPosition().copy(), player.getUsername(), player.getHostAdress(), false, 120, player.getPosition().getZ() >= 0 && player.getPosition().getZ() < 4 ? true : false, 60));
			...
			while (amount > 0) {
				...
				if (slot == -1) {
					GroundItemManager.spawnGroundItem(player, new GroundItem(new Item(item.getId(), 1), player.getPosition().copy(), player.getUsername(), false, 120, player.getPosition().getZ() >= 0 && player.getPosition().getZ() < 4 ? true : false, 60));
					dropped = true;
				} else {
					items[slot].setId(item.getId());
		...
		if (refresh)
			refreshItems();
		if(dropped && getPlayer().getRights() != PlayerRights.ADMINISTRATOR && getPlayer().getRights() != PlayerRights.OWNER && getPlayer().getRights() != PlayerRights.DEVELOPER)
	 ...
	 * @return			The ItemContainer instance.
	 */
	public boolean addToContainer(Item item, boolean refresh) {
		if (item == null)
			return false;
		if (ItemDefinition.forId(item.getId()).isStackable() || stackType() == StackType.STACKS) {
			int slot = getSlot(item.getId());
