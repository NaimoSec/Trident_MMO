...
import org.trident.engine.task.impl.FireImmunityTask;
import org.trident.engine.task.impl.OverloadPotionTask;
import org.trident.engine.task.impl.PoisonImmunityTask;
...
public class Consumables {
	...
	public static void handleHealAction(Player player) {
		...
		for(Item item : player.getInventory().getItems()) {
			if(item != null) {
				if(isFood(player, item, player.getInventory().getSlot(item.getId()))) {
					return;
				}
			}
		}
	...
	private static void eat(Player player, FoodType food, int slot) {
		...
		if(player.getLocation() == Location.DUEL_ARENA && player.getDueling().duelingStatus == 5) {
			if(player.getDueling().selectedDuelRules[Dueling.DuelRule.NO_FOOD.ordinal()]) {
				player.getPacketSender().sendMessage("Food has been disabled in this duel.");			
				return;
			}
		}
		if (food != null && (System.currentTimeMillis() - player.getAttributes().getFoodDelay()) >= 1800) {
			player.getPacketSender().sendInterfaceRemoval();
			CombatHandler.resetAttack(player);
			player.getCombatAttributes().setAttackDelay(player.getCombatAttributes().getAttackDelay() + 2);
			...
			SoundEffects.sendSoundEffect(player, SoundEffects.SoundData.EAT_FOOD, 10, 2);
	...
	public static void handlePotion(final Player player, final int itemId, final int slot) {
		if(player.getConstitution() <= 0)
			return;
		if(player.getLocation() == Location.DUEL_ARENA && player.getDueling().duelingStatus == 5) {
			if(player.getDueling().selectedDuelRules[Dueling.DuelRule.NO_FOOD.ordinal()] && healingPotion(itemId)) {
				player.getPacketSender().sendMessage("Since food has been disabled in this duel, health-healing potions won't work.");			
				return;
			}
			if(player.getDueling().selectedDuelRules[Dueling.DuelRule.NO_POTIONS.ordinal()]) {
				player.getPacketSender().sendMessage("Potions have been disabled in this duel.");			
				return;
			}
		}
		if (System.currentTimeMillis() - player.getAttributes().getPotionDelay() >= 1500) {
			switch(itemId) {
			/*
			 * Attack potions
			 */
			...
			case 3008: //Energy potion 4
				...
				player.getInventory().refreshItems();
				player.getAttributes().setRunEnergy(player.getAttributes().getRunEnergy() + (itemId == 3008 ? 15 : 40));
			...
			case 3010: //Energy potion 3
				...
				player.getInventory().getItems()[slot] = new Item(itemId+2, 1);
				player.getInventory().refreshItems();
				player.getAttributes().setRunEnergy(player.getAttributes().getRunEnergy() + (itemId == 3010 ? 15 : 40));
				if(player.getAttributes().getRunEnergy() > 100)
					player.getAttributes().setRunEnergy(100);
				break;
			...
			case 3012: //Energy potion 2
				...
				player.getInventory().getItems()[slot] = new Item(itemId+2, 1);
				player.getInventory().refreshItems();
				player.getAttributes().setRunEnergy(player.getAttributes().getRunEnergy() + (itemId == 3012 ? 15 : 40));
				if(player.getAttributes().getRunEnergy() > 100)
					player.getAttributes().setRunEnergy(100);
				break;
			case 3022:
			case 3014: //Energy potion 1
				...
				player.getInventory().getItems()[slot] = new Item(229, 1);
				player.getInventory().refreshItems();
				player.getAttributes().setRunEnergy(player.getAttributes().getRunEnergy() + (itemId == 3014 ? 15 : 40));
				if(player.getAttributes().getRunEnergy() > 100)
					player.getAttributes().setRunEnergy(100);
				break;
				/*
			...
			case 3024:
				...
				for(int i = 0; i <= 24; i++) {
					if(i == 3)
						continue;
					if(player.getSkillManager().getCurrentLevel(Skill.forId(i)) < player.getSkillManager().getMaxLevel(i)) {
						player.getSkillManager().setCurrentLevel(Skill.forId((i)), (int) (player.getSkillManager().getCurrentLevel(Skill.forId(i)) + (player.getSkillManager().getMaxLevel(Skill.forId(i)) * 0.18)));
						if(player.getSkillManager().getCurrentLevel(Skill.forId(i)) > player.getSkillManager().getMaxLevel(Skill.forId(i))) 
							player.getSkillManager().setCurrentLevel(Skill.forId(i), player.getSkillManager().getMaxLevel(Skill.forId(i)));
					}
				}
			...
			case 6685:
				...
				double amount = player.getEquipment().wearingTorva() ? 1.22 : 1.17;
				int bonus = player.getEquipment().wearingTorva() ? getBrewStat(player, 3, .21) : getBrewStat(player, 3, .15);
			...
			case 6687:
				...
				player.getSkillManager().updateSkill(Skill.forId(1));
				amount = player.getEquipment().wearingTorva() ? 1.22 : 1.17;
				bonus = player.getEquipment().wearingTorva() ? getBrewStat(player, 3, .21) : getBrewStat(player, 3, .15);
				player.getSkillManager().setCurrentLevel(Skill.forId(3), player.getSkillManager().getCurrentLevel(Skill.forId(3)) + bonus);
				if (player.getSkillManager().getCurrentLevel(Skill.forId(3)) > ( player.getSkillManager().getMaxLevel(Skill.forId(3)) * amount + 1)) {
					player.getSkillManager().setCurrentLevel(Skill.forId(3), (int) (player.getSkillManager().getMaxLevel(Skill.forId(3)) * amount));
				}
			...
			case 15300:
				...
				if(System.currentTimeMillis() - player.getAttributes().getSpecialRestoreTimer() < 30000) {
				...
				player.getInventory().refreshItems();
				player.getPlayerCombatAttributes().setSpecialAttackAmount(player.getPlayerCombatAttributes().getSpecialAttackAmount() + 2.5);
				if(player.getPlayerCombatAttributes().getSpecialAttackAmount() > 10.00) 
					player.getPlayerCombatAttributes().setSpecialAttackAmount(10.00);
				WeaponHandler.update(player);
				player.getAttributes().setSpecialRestoreTimer(System.currentTimeMillis());
				break;
			case 15301:
				if(player.getLocation() != null && player.getLocation() == Location.WILDERNESS) {
					player.getPacketSender().sendMessage("This potion cannot be used in the Wilderness.");
			...
			case 21630:
				...
				player.getInventory().refreshItems();
				player.getAttributes().setPrayerRenewalPotionTimer(600);
			...
			player.getPacketSender().sendInterfaceRemoval();
			CombatHandler.resetAttack(player);
			player.getCombatAttributes().setAttackDelay(player.getCombatAttributes().getAttackDelay() + 1);
			player.getAttributes().setPotionDelay(System.currentTimeMillis());
			player.getAttributes().setFoodDelay(player.getAttributes().getPotionDelay());
			...
			if(player.getAttributes().getOverloadPotionTimer() > 0) {  // Prevents decreasing stats
			...
			SoundEffects.sendSoundEffect(player, SoundEffects.SoundData.DRINK_POTION, 10, 2);
		}
	}
	...
	public static boolean drinkOverload(final Player player, int slot, int replacePotion) {
		...
		player.getInventory().getItems()[slot] = new Item(replacePotion, 1);
		player.getInventory().refreshItems();
		player.getAttributes().setOverloadPotionTimer(600);
