...
/**
 * Handles the enchantment spells in the modern magic spellbook.
 * @author phl0w
 */
public class Enchanting {
	...
	public static boolean enchantItem(Player p, int itemID, int spellID) {
		...
		if(enc == null) {
			p.getPacketSender().sendMessage("This spell cannot be cast on this item.");
			return false;
		}
		if (p.getSkillManager().getCurrentLevel(Skill.MAGIC) >= enc.getLevelReq()) {
			if (p.getInventory().contains(itemID) && p.getInventory().contains(enc.getUnenchanted())) {
				if (hasRunes(p, spellID)) {
					if (getEnchantmentLevel(spellID) == enc.getELevel()) {
						p.getInventory().delete(enc.getUnenchanted(), 1).delete(ens.getReq1(), ens.getReqAmt1()).delete(ens.getReq2(), ens.getReqAmt2()).add(enc.getEnchanted(), 1);
						p.getSkillManager().addExperience(Skill.MAGIC, enc.getXp(), false);
						p.performAnimation(new Animation(enc.getAnim()));
						p.performGraphic(new Graphic(enc.getGFX(), GraphicHeight.HIGH));
						if (ens.getReq3() != -1)
							p.getInventory().delete(ens.getReq3(), ens.getReqAmt3());
						p.getPacketSender().sendTab(Constants.MAGIC_TAB);
					} else {
						p.getPacketSender().sendMessage("You can only enchant this jewelry using a level-"+enc.getELevel()+" enchantment spell.");
					}
				} else {
					p.getPacketSender().sendMessage("You do not have enough runes in your inventory to cast this spell.");
				}
			}
		} else {
			p.getPacketSender().sendMessage("You need a Magic level of at least "+enc.getLevelReq()+" to cast this spell.");	
		}
		return true;
	}

	private static boolean hasRunes(Player p, int spellID) {
		EnchantSpell ens = EnchantSpell.forId(spellID);
		if (ens.getReq3() == 0) {
			return p.getInventory().getAmount(ens.getReq1()) >= ens.getReqAmt1() && p.getInventory().getAmount(ens.getReq2()) >= ens.getReqAmt2() && p.getInventory().getAmount(ens.getReq3()) >= ens.getReqAmt3();
		} else {
			return p.getInventory().getAmount(ens.getReq1()) >= ens.getReqAmt1() && p.getInventory().getAmount(ens.getReq2()) >= ens.getReqAmt2();
		}
	}
}
