...
import org.trident.engine.task.impl.CeilingCollapseTask;
...
public class Barrows {
	...
	public static boolean handleObject(final Player player, GameObject object) {
		switch(object.getId()) {
		case 6771:
			searchCoffin(player, object.getId(), 4, CustomNPCData.DHAROK_THE_WRETCHED, object.getPosition() != null ? new Position(3557, 9715, player.getPosition().getZ()) : new Position(3552, 9693));
			return true;
		case 6823:
			searchCoffin(player, object.getId(), 0, CustomNPCData.VERAC_THE_DEFILED, object.getPosition() != null ? new Position(3575, 9704, player.getPosition().getZ()) : new Position(3552, 9693));
			return true;
		case 6821:
			searchCoffin(player, object.getId(), 5, CustomNPCData.AHRIM_THE_BLIGHTED, object.getPosition() != null ? new Position(3557, 9699, player.getPosition().getZ()) : new Position(3552, 9693));
			return true;
		case 6772:
			searchCoffin(player, object.getId(), 1, CustomNPCData.TORAG_THE_CORRUPTED, object.getPosition() != null ? new Position(3571, 9684, player.getPosition().getZ()) : new Position(3552, 9693));
			return true;
		case 6822:
			searchCoffin(player, object.getId(), 2, CustomNPCData.KARIL_THE_TAINTED, object.getPosition() != null ? new Position(3549, 9681, player.getPosition().getZ()) : new Position(3552, 9693));
			return true;
		case 6773:
			searchCoffin(player, object.getId(), 3, CustomNPCData.GUTHAN_THE_INFESTED, object.getPosition() != null ? new Position(3537, 9703, player.getPosition().getZ()) : new Position(3552, 9693));
			return true;
		case 6745:
		...
		case 10284:
			if(player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getKillcount() < 5) 
				return true;
			if (player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getRandomCoffin()][1] == 0) {
				handleObject(player, new GameObject(COFFIN_AND_BROTHERS[player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getRandomCoffin()][0], null));
				player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getRandomCoffin()][1] = 1;
				return true;
			} else if (player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getRandomCoffin()][1] == 1) {
				player.getPacketSender().sendMessage("You cannot loot this wilst in combat!");
				return true;
			} else if (player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getRandomCoffin()][1] == 2 && player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getKillcount() >= 6) {
				if(player.getInventory().getFreeSlots() < 2) {
					player.getPacketSender().sendMessage("You need at least 2 free inventory slots to loot this chest.");
					return true;
				}
				int bonusKillcountRate = (int) ((player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getKillcount() - 7) * 0.07);
				if(bonusKillcountRate < 0) 
					bonusKillcountRate = 0;
				resetBarrows(player);
				player.getInventory().add(randomRunes(),  25 + Misc.getRandom(70));
				if (Misc.getRandom(15) - bonusKillcountRate <= 1)
					player.getInventory().add(randomBarrows(), 1);
	...
	public static void showRiddle(Player player) {
		...
		player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().setRiddleAnswer(riddles[riddle][7]);
		player.getPacketSender().sendInterface(4543);
	}

	public static void handlePuzzle(Player player, int puzzleButton) {
		if(puzzleButton == player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getRiddleAnswer()) {
			player.moveTo(new Position(3551, 9694));
		...
		player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().setRiddleAnswer(-1);
	}
	 ...
	 */
	public static void searchCoffin(final Player player, final int obj, final int coffinId, CustomNPCData npcData, Position spawnPos) {
		player.getPacketSender().sendInterfaceRemoval();
		if(player.getPosition().getZ() == -1) {
			if (selectCoffin(player, obj))
				return;
		}
		if (player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[coffinId][1] == 0) {
			if(player.getLocation() == Location.BARROWS) {
				player.getAttributes().setRegionInstance(new RegionInstance(player, RegionInstanceType.BARROWS));
				NPC npc_ = NPCSpawns.createCustomNPC(npcData, spawnPos);
				npc_.forceChat(player.getPosition().getZ() == -1 ? "You dare disturb my rest!" : "You dare steal from us!");
				npc_.getCombatAttributes().setAttackDelay(3);
				...
				player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[coffinId][1] = 1;
			}
		} else {
	...
	public static void resetBarrows(Player player) {
		player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().setKillcount(0);
		for(int i = 0; i < player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData().length; i++)
			player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[i][1] = 0;
		updateInterface(player);
		player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().setRandomCoffin(getRandomCoffin());
	}
	...
	public static void killBarrowsNpc(Player player, NPC n, boolean killed) {
		if(player == null || n == null)
			return;
		if(n.getId() == 58) {
			player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().setKillcount(player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getKillcount()+1);
			updateInterface(player);
			return;
		}
		...
		if(arrayIndex < 0)
			return;
		if(killed) {
			player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[arrayIndex][1] = 2;
			player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().setKillcount(player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getKillcount()+1);
			player.getAttributes().getRegionInstance().getNpcsList().remove(player);
		} else
			if(arrayIndex >= 0)
				player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[arrayIndex][1] = 0;
		updateInterface(player);
	}
	...
	public static boolean selectCoffin(Player player, int coffinId) {
		if (player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getRandomCoffin() == 0)
			player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().setRandomCoffin(getRandomCoffin());
		if (COFFIN_AND_BROTHERS[player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getRandomCoffin()][0] == coffinId) {
			DialogueManager.start(player, 255);
			player.getAttributes().setDialogueAction(222);
			return true;
		}
		return false;
	}
	...
	public static int getBarrowsIndex(Player player, int id) {
		int index = -1;
		for(int i = 0; i < player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData().length; i++) {
			if(player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[i][0] == id) {
				index = i;
			}
		}
		return index;
	...
	public static void updateInterface(Player player) {
		for(int i = 0; i < data.length; i++) {
			boolean killed = player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[i][1] == 2; String s = killed ? "@gre@" : "@red@";
			player.getPacketSender().sendString((int)data[i][1], ""+s+""+(String)data[i][0]);
		}
		player.getPacketSender().sendString(37208, "Killcount: "+player.getAttributes().getMinigameAttributes().getBarrowsMinigameAttributes().getKillcount());
	}
