...
public class SummoningData {
	...
	public static boolean isPouch(Player player, Item item) {
		FamiliarData familiar = FamiliarData.forId(item.getId());
		if (familiar != null) {
			boolean renew = player.getAdvancedSkills().getSummoning().getFamiliar() != null && Summoning.FamiliarData.forNPCId(player.getAdvancedSkills().getSummoning().getFamiliar().getSummonNpc().getId()).getPouchId() == item.getId();
			player.getAdvancedSkills().getSummoning().summon(familiar, renew, false);
			return true;
		}
	...
	public static int calculateScrolls(Player player) {
		if(player.getAdvancedSkills().getSummoning().getFamiliar() != null) {
			int scrollId = Summoning.FamiliarData.forNPCId(player.getAdvancedSkills().getSummoning().getFamiliar().getSummonNpc().getId()).scroll;
