...
public class CurseHandler {
	...
	public static void activateCurse(Player player, int curseId) {
		if(player.getAttributes().getPrayerbook() == Prayerbook.NORMAL)
			return;
	...
	public static void activateCurse(Player player, CurseData curse) {
		...
		if(player.getLocation() == Location.DUEL_ARENA && player.getDueling().duelingStatus == 5) {
			if(player.getDueling().selectedDuelRules[Dueling.DuelRule.NO_PRAYER.ordinal()]) {
		...
		if (player.getAttributes().isPrayerInjured()) {
		...
		if (noActiveCurse(player, curse) && !player.getAttributes().isDrainingPrayer())
			startDrain(player);
		BonusManager.sendCurseBonuses(player);
		if(player.getAttributes().getInterfaceId() == 17100 && curse == CurseData.PROTECT_ITEM)
	...
	private static void startDrain(final Player player) {
		if (getDrain(player) <= 0 || player.getAttributes().isDrainingPrayer())
			return;
		player.getAttributes().setDrainingPrayer(true);
		TaskManager.submit(new Task(1, player, true) {
			...
			public void execute() {
				...
				if (player.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 0) {
					...
					SoundEffects.sendSoundEffect(player, SoundEffects.SoundData.RUN_OUT_OF_PRAYER_POINTS, 10, 0);
					stop();
					return;
				}
			...
			public void stop() {
				...
				player.getAttributes().setDrainingPrayer(false);
			}
		});
	}
	...
	private enum CurseData {
		PROTECT_ITEM(50, .6, 32503, 610, new PrayerAnimation(new Animation(12567), new Graphic(2213))),
		SAP_WARRIOR(50, .9, 32505, 611),
		SAP_RANGER(52, .9, 32507, 612),
		SAP_MAGE(54, .9, 32509, 613),
		SAP_SPIRIT(56, .9, 32511, 614),
		BERSERKER(59, 1.2, 32513, 615, new PrayerAnimation(new Animation(12589), new Graphic(2266))),
		DEFLECT_SUMMONING(62, 1.25, 32515, 616),
		DEFLECT_MAGIC(65, 1.5, 32517, 617),
		DEFLECT_MISSILES(68, 1.5, 32519, 618),
		DEFLECT_MELEE(71, 1.5, 32521, 619),
		LEECH_ATTACK(74, 2.5, 32523, 620),
		LEECH_RANGED(76, 2.5, 32525, 621),
		LEECH_MAGIC(78, 2.5, 32527, 622),
		LEECH_DEFENCE(80, 2.5, 32529, 623),
		LEECH_STRENGTH(82, 2.5, 32531, 624),
		LEECH_ENERGY(84, 2.5, 32533, 625),
		LEECH_SPECIAL_ATTACK(86, 2.5, 32535, 626),
		WRATH(89, 2, 32537, 627),
		SOUL_SPLIT(92, 4, 32539, 628),
