...
public class SummoningTab {
	...
	public static void handleDismiss(Player c, boolean dismiss) {
		if(!c.getAttributes().hasStarted()) //didnt finish tut
			return;
		c.getPacketSender().sendInterfaceRemoval();
		if(dismiss) {
			if(c.getAdvancedSkills().getSummoning().getFamiliar() != null) {
				c.getAdvancedSkills().getSummoning().resetFollower(true);
			} else {
				c.getPacketSender().sendMessage("You don't have a familiar to dismiss.");
			}
		} else {
			DialogueManager.start(c, 30);
			c.getAttributes().setDialogueAction(17);
		}
	}
	...
	public static void handleAttack(Player c) {
		if(c.getAdvancedSkills().getSummoning().getFamiliar() != null) {
			c.getPacketSender().sendMessage("Your familiar will automatically aid you in combat when in a multi zone.");
		} else {
			c.getPacketSender().sendMessage("You do not have a familiar.");
		}
	}
	...
	public static void callFollower(final Player c) {
		if(c.getAdvancedSkills().getSummoning().getFamiliar() != null && c.getAdvancedSkills().getSummoning().getFamiliar().getSummonNpc() != null) {
			if(System.currentTimeMillis() - c.getAttributes().getSummoningSpawnDelay() < 30000) {
				c.getPacketSender().sendMessage("You must wait another "+Misc.getTimeLeft(c.getAttributes().getSummoningSpawnDelay(), 30, TimeUnit.SECONDS)+" before being able to do this again.");
				return;
			}
			c.getAdvancedSkills().getSummoning().moveFollower();
			c.getPacketSender().sendMessage("You've called your familiar.");
			c.getAttributes().setSummoningSpawnDelay(System.currentTimeMillis());
		} else {
	...
	public static void renewFamiliar(Player c) {
		if(c.getAdvancedSkills().getSummoning().getFamiliar() != null) {
			int pouchRequired = Summoning.FamiliarData.forNPCId(c.getAdvancedSkills().getSummoning().getFamiliar().getSummonNpc().getId()).getPouchId();
			if(c.getInventory().contains(pouchRequired)) {
				c.getAdvancedSkills().getSummoning().summon(Summoning.FamiliarData.forNPCId(c.getAdvancedSkills().getSummoning().getFamiliar().getSummonNpc().getId()), true, false);
			} else {
