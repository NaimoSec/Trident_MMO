...
import org.trident.model.inputhandling.impl.EnterAmountOfHidesToTan;
	...
	public static void tanHide(final Player player, final int buttonId, int amount) {
		for (final tanningData t : tanningData.values()) {
			if (buttonId == t.getButtonId(buttonId)) {
				...
				boolean usePouch = player.getAttributes().getMoneyInPouch() > price;
				int coins = usePouch ? player.getAttributes().getMoneyInPouch() : player.getInventory().getAmount(995);
				if (coins == 0) {
				...
				if (coins >= price) {
					if (player.getInventory().contains(hide)) {
						...
						if(usePouch) {
							player.getAttributes().setMoneyInPouch(player.getAttributes().getMoneyInPouch() - price);
		...
		for (tanningData t : tanningData.values()) {
			if (id == t.getButtonId(id)) {
				if(t.getAmount(id) == 29) {
					player.getAttributes().setInputHandling(new EnterAmountOfHidesToTan(id));
