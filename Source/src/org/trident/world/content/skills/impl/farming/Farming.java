...
public class Farming {
	...
	private Plant[] plants = new Plant[3];
	private GrassyPatch[] patches = new GrassyPatch[3];
					...
					if (plant.seed == seed) {
						if (player.getSkillManager().getCurrentLevel(Skill.FARMING) >= plant.level) {
							...
							if (player.getInventory().contains(patch.planter)) {
								...
								insert(planted);
								doConfig();
								player.getSkillManager().addExperience(Skill.FARMING, (int)plant.plantExperience, false);
							} else {
	...
	public void save(Player p) {
		if(!p.getAttributes().shouldProcessFarming())
			return;
		try {
			BufferedWriter writer = new BufferedWriter(new FileWriter(DIR + ""+p.getUsername()+".txt"));
			for (int i = 0; i < patches.length; i++) {
				if (i >= FarmingPatches.values().length)
					break;
				if (patches[i] != null) {
	...
	public void load(Player p) {
		if(!p.getAttributes().shouldProcessFarming())
			return;
		try {
			BufferedReader r = new BufferedReader(new FileReader(DIR + ""+p.getUsername()+".txt"));
