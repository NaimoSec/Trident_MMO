...
public class Herblore {
	...
	public static boolean cleanHerb(final Player player, final int herbId) {
		...
		if(player.getInventory().contains(herb.getGrimyHerb())) {
			...
			player.getSkillManager().addExperience(Skill.HERBLORE, herb.getExp(), false);
		...
		if(player.getInventory().contains(pot.getUnfinishedPotion()) && player.getInventory().contains(pot.getItemNeeded())) {
			...
			TaskManager.submit(new Task(1, player, false) {
				public void execute() {
					...
					player.getSkillManager().addExperience(Skill.HERBLORE, (pot.getExpGained() * 8), false);
	...
	public static void handleSpecialPotion(Player p, int item1, int item2) {
		...
		if(System.currentTimeMillis() - p.getAttributes().getClickDelay() <= 500)
			return;
		...
		p.getSkillManager().addExperience(Skill.HERBLORE, specialPotData.getExperience(), false);
