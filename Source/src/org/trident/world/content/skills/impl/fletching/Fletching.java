package org.trident.world.content.skills.impl.fletching;
...
public class Fletching {
	...
	public static void openSelection(final Player player, int log) {
		player.getSkillManager().stopSkilling().getSkillAttributes().setSelectedItem(log);
		...
		if(log == 1511) {
			...
			player.getPacketSender().createFrame(8866);
	...
	public static boolean fletchingButton(final Player player, int button) {
		switch (button) {
		case 8889:
			switch (player.getSkillManager().getSkillAttributes().getSelectedItem()) {
		...
		case 8895: //Arrow shafts
			if(player.getSkillManager().getSkillAttributes().getSelectedItem() == 1511) {
	...
	public static void fletchBow(final Player player, final int product, final int amountToMake) {
		...
		player.getSkillManager().getSkillAttributes().setCurrentTask(new Task(2, player, true) {
			...
			public void execute() {
				...
				player.getSkillManager().addExperience(Skill.FLETCHING, shafts ? 80 : (int) (bow.getXp() * 1.5), false);
				SoundEffects.sendSoundEffect(player, SoundEffects.SoundData.FLETCH_ITEM, 10, 4);
				amount++;
	...
	public static void openBowStringSelection(Player player, int log) {
		for (final StringingData g : StringingData.values()) {
			if (log == g.unStrung()) {
				player.getSkillManager().stopSkilling().getSkillAttributes().setSelectedItem(log);
				player.getAttributes().setInputHandling(new EnterAmountOfBowsToString());
			...
			if (log == g.unStrung()) {
				...
				player.getSkillManager().getSkillAttributes().setCurrentTask(new Task(2, player, false) {
					...
					public void execute() {
						...
						player.getSkillManager().addExperience(Skill.FLETCHING, (int) g.getXP(), false);
						amountMade++;
		...
		if (arr != null) {
			if (player.getSkillManager().getCurrentLevel(Skill.FLETCHING) >= arr.getLevelReq()) {
				if (player.getInventory().getAmount(arr.getItem1()) >= 15 && player.getInventory().getAmount(arr.getItem2()) >= 15) {
					...
					player.getSkillManager().addExperience(Skill.FLETCHING, arr.getXp(), false);
				} else {
