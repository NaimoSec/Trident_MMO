...
import org.trident.model.inputhandling.impl.EnterAmountOfBarsToSmelt;
...
public class SmithingData {
	...
	public static final int[][] SmeltData = {
		...
		{2363, 451, 453, 85, 550} // Runite bar
	};

	public static int[] ores = {-1, -1};
	...
	public static boolean hasOres(Player player, int barId) {
		getOres(barId); //Insert ores ids to the array
		if(ores[0] > 0 && ores[1] < 0) {
			if(player.getInventory().getAmount(ores[0]) > 0)
				return true;
		} else if(ores[1] > 0 && ores[1] != 453 && ores[0] > 0) {
			if(player.getInventory().getAmount(ores[1]) > 0 && player.getInventory().getAmount(ores[0]) > 0)
				return true;
		} else if(ores[1] > 0 && ores[1] == 453 && ores[0] > 0) {
			if(player.getInventory().getAmount(ores[1]) >= getCoalAmount(barId) && player.getInventory().getAmount(ores[0]) > 0)
				return true;
		}
		return false;
	...
	public static boolean canSmelt(Player player, int barId) {
		...
		if(!hasOres(player, barId)) {
			...
			if(ores[0] > 0 && ores[1] > 0 && ores[1] != 453) {
				requirement = "To make "+anOrA(barId)+" "+new Item(barId).getDefinition().getName()+", you need some "+new Item(ores[0]).getDefinition().getName().replace(" ore", "")+" and "+new Item(ores[1]).getDefinition().getName()+".";
			} else if(ores[0] > 0 && ores[1] == -1) {
				requirement = "To make "+anOrA(barId)+" "+new Item(barId).getDefinition().getName()+", you need some "+new Item(ores[0]).getDefinition().getName()+".";
			} else if(ores[0] > 0 && ores[1] == 453) { //The bar uses custom coal amount
				requirement = "To make "+anOrA(barId)+" "+new Item(barId).getDefinition().getName()+", you need some "+new Item(ores[0]).getDefinition().getName().replace(" ore", "")+" and "+getCoalAmount(barId)+" "+new Item(ores[1]).getDefinition().getName()+" ores.";
			}
	...
	public static void handleButtons(Player player, int id) {
		...
		case 7447:
			...
			if(bar > 0) {
				player.getAttributes().setInputHandling(new EnterAmountOfBarsToSmelt(bar));
	...
	public static void sendString(Player player, String s, int i) { // lol cba converting
