...
public class PrayerHandler {
	...
	public static void activatePrayer(Player player, final int prayerId) {
		if(player.getAttributes().getPrayerbook() == Prayerbook.CURSES)
			return;
		...
		if (hasNoPrayerOn(player, prayerId) && !player.getAttributes().isDrainingPrayer())
			startDrain(player);
		if (pd.hint != -1) {
	 ...
	 */
	private static final int[] DEFENCE_PRAYERS = {THICK_SKIN, ROCK_SKIN, STEEL_SKIN, CHIVALRY, PIETY};
	 ...
	 */
	private static final int[] RANGED_PRAYERS = {SHARP_EYE, HAWK_EYE, EAGLE_EYE};
	
	/**
	 * Contains every prayer that counts as a magic prayer.
	 */
	private static final int[] MAGIC_PRAYERS = {MYSTIC_WILL, MYSTIC_LORE, MYSTIC_MIGHT};
	 ...
	 */
	public static void switchPrayerbook(Player player, boolean message) {
		if(player.getAttributes().getPrayerbook() == Prayerbook.NORMAL) {
			...
			player.getAttributes().setPrayerbook(Prayerbook.CURSES);
		} else {
			if(message)
				player.getPacketSender().sendMessage("You sense a surge of purity flow through your body!");
			player.getAttributes().setPrayerbook(Prayerbook.NORMAL);
		}
