...
public class ItemBinding {
	...
	public static void unbindItem(Player p, int item) {
		if(Dungeoneering.doingDungeoneering(p)) {
			for(int i = 0; i  < p.getSkillManager().getSkillAttributes().getDungeoneeringAttributes().getBoundItems().length; i++) {
				if(p.getSkillManager().getSkillAttributes().getDungeoneeringAttributes().getBoundItems()[i] == item) {
					p.getSkillManager().getSkillAttributes().getDungeoneeringAttributes().getBoundItems()[i] = -1;
					p.getInventory().delete(item, 1);
	...
	public static void bindItem(Player p, int item) {
		if(Dungeoneering.doingDungeoneering(p)) {
			...
			int amountBound = 0;
			for(int i = 0; i  < p.getSkillManager().getSkillAttributes().getDungeoneeringAttributes().getBoundItems().length; i++) {
				if(p.getSkillManager().getSkillAttributes().getDungeoneeringAttributes().getBoundItems()[i] > 0)
					amountBound++;
			}
			if(amountBound >= 4) {
				...
				return;
			} else if(amountBound == 2 && p.getSkillManager().getCurrentLevel(Skill.DUNGEONEERING) < 100) {
				p.getPacketSender().sendMessage("You need a Dungeoneering level of at least 100 to have 3 bound items.");
				return;
			...
			if(bind != -1 && index != -1) {
				p.getSkillManager().getSkillAttributes().getDungeoneeringAttributes().getBoundItems()[index] = bind;
	...
	public static void onDungeonEntrance(Player p) {
		for(int i = 0; i < p.getSkillManager().getSkillAttributes().getDungeoneeringAttributes().getBoundItems().length; i++) {
			if(p.getSkillManager().getSkillAttributes().getDungeoneeringAttributes().getBoundItems()[i] > 0)
				p.getInventory().add(p.getSkillManager().getSkillAttributes().getDungeoneeringAttributes().getBoundItems()[i], 1);
		}
	}
}
